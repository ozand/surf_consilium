...
class SubprocessRunner:
    ...
    def __init__(self, executable: str = "surf", output_limit: int = 50_000):
...
class ClaudeSurfAdapter:
    ...
    def _surf(self, args: list[str], timeout: float | None = None) -> CommandResult:
        prefix = ["--window-id", self.window_id] if self.window_id else []
    ...
    def __call__(self, prompt: str) -> ProviderResult:
        ...
        for _ in range(self.poll_attempts):
            ...
            if answer and "Stop response" not in last_read and "Claude is responding" not in last_read:
